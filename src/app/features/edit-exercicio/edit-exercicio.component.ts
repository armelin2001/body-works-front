...
import {
  ...
  TIPO_EXERCICIO,
  NIVEL_DIFICULDADE,
} from 'src/app/shared/models/exercicio.dto';
...
export class EditExercicioComponent implements OnInit {
      ...
      this.exercicioService.obterExercicioPorId(this.id).subscribe(
        (res) => {
          this.formularioExercicio.patchValue({
            nome: res.nome,
            tipoExercicio: res.tipoExercicio,
  ...
  get tipoExercicio(): FormControl {
    const tipoExercicio = this.formularioExercicio.get(
      'tipoExercicio'
    ) as FormControl;
  ...
  get equipamentoNecessario(): FormControl {
    const equipamentoNecessario = this.formularioExercicio.get(
  ...
  get nivelDificuldade(): FormControl {
    const nivelDificuldade = this.formularioExercicio.get(
      'nivelDificuldade'
    ) as FormControl;
  ...
  get videoDemonstrativo(): FormControl {
    const videoDemonstrativo = this.formularioExercicio.get(
      'videoDemonstrativo'
    ) as FormControl;
  ...
  get musculosTrabalhados(): FormControl {
    const musculosTrabalhados = this.formularioExercicio.get(
      'musculosTrabalhados'
    ) as FormControl;
